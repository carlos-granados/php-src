...
static void zend_create_closure_ex(zval *res, zend_function *func, zend_class_entry *scope, zend_class_entry *called_scope, zval *this_ptr, bool is_fake) /* {{{ */
{
	...
	if (func->type == ZEND_USER_FUNCTION) {
		...
		/* A generic clone's private (or dedup-cache-SHARED) arg_info is
		 * owned by the source function (class table / EG(function_table)
		 * monomorph, or EG(subst_arg_info_cache)), which outlives any
		 * closure over it; the copy merely borrows it, so it must not carry
		 * either flag that makes destroy_op_array touch it (release
		 * contents, or even just detach the pointer -- both would race the
		 * source's own, still-pending teardown/cache-release). */
		closure->func.common.fn_flags2 &= ~ZEND_ACC2_GENERIC_ARGINFO_ANY_CLONE;
